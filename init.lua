...
local function scan_region_for_node(state, regions, r, pos, node_name, user_action, show_log)
	...
	if count > 0 or user_action then
		...
		scan_nearby_region(state, r, vector.new(0, 0, -size.z), "Z-", pos, node_name)
		scan_nearby_region(state, r, vector.new(size.x, 0, size.z), "North East", pos, node_name)
		scan_nearby_region(state, r, vector.new(size.x, size.y, size.z), "North East Up", pos, node_name)
		scan_nearby_region(state, r, vector.new(size.x, size.y, 0), "East Up", pos, node_name)
		scan_nearby_region(state, r, vector.new(0, size.y, size.z), "North Up", pos, node_name)
		scan_nearby_region(state, r, vector.new(-size.x, 0, size.z), "North West", pos, node_name)
		scan_nearby_region(state, r, vector.new(-size.x, -size.y, 0), "West Down", pos, node_name)
		scan_nearby_region(state, r, vector.new(size.x, 0, -size.z), "South East", pos, node_name)
		scan_nearby_region(state, r, vector.new(0, size.y * 2, 0), "Up x2", pos, node_name)
	end
end
